...
//+PLUMEDOC DIMRED TICA_old 
/* 
Time-lagged independent component analysis (TICA_old) using a large number of collective variables as input.


\par Examples

A simple example is as below, it will use the TICA_old method to analyze the CVs cv1 and cv2 from metadynamics simulaiton (COLVAR file colvar.0.data)
...
After using TICA_old method, you will also get the correlation file (default name correlation.data).
...
class TICA_old : public vesselbase::ActionWithAveraging {
private:
...
public:
	...
	explicit TICA_old( const ActionOptions& );
	~TICA_old();
	void calculate(){}
	void apply(){}
...
PLUMED_REGISTER_ACTION(TICA_old,"TICA_OLD")

void TICA_old::registerKeywords( Keywords& keys ){
	vesselbase::ActionWithAveraging::registerKeywords( keys );
	keys.remove("SERIAL"); keys.remove("LOWMEM"); 
	keys.remove("ARG");
	keys.add("compulsory","ARG","the CVs for the input to calculate TICA_old"); 
  	keys.add("compulsory","DELTA_TAU","the delta lagged time to calculate TICA_old");
...
TICA_old::TICA_old(const ActionOptions&ao):
Action(ao),
...
data(getNumberOfArguments())
{
	...
	if(corr_input.size()>0)
	{
		...
		for(unsigned ip=0;ip!=npoints;++ip)
		{
			...
			plumed_massert(fabs(ip*delta_tau*dt-lag_time)<1.0e-6,"the lagged time read from the correlation file mismatch!");
...
void TICA_old::performAnalysis()
{
	...
	for(unsigned ip=0;ip!=npoints;++ip)
	{
		...
		ocorr.printField("LAGGED_TIME",time);
		ocorr2.printField("LAGGED_TIME",time);
...
void TICA_old::performOperations( const bool& from_update ){
  accumulate();
}

void TICA_old::runAnalysis(){
	performAnalysis(); idata=0;
}

void TICA_old::runFinalJobs() {
  runAnalysis(); 
}

Matrix<double> TICA_old::build_correlation_integer(double& unorm,unsigned _tau)
{
...
Matrix<double> TICA_old::build_correlation_float(double& fnorm,double _tau)
{
...
double TICA_old::get_correlation_integer(const std::vector<double>& X,const std::vector<double>& Y,const std::vector<bool>& ynan,unsigned _tau,double unorm)
{
	double corrmean=0;
	if(ignore_reweight)
	{
...
double TICA_old::get_correlation_float(const std::vector<double>& X,const std::vector<double>& Y,const std::vector<unsigned>& xid,const std::vector<unsigned>& yid,const std::vector<double>& twt,double fnorm)
{
	double corrmean=0;
